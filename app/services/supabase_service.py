...
class SupabaseService:
    ...
    def has_followup(self, row_number: int, date_str: str) -> bool:
        resp = supabase.table("followup_logs").select("id").eq("sheet_row", row_number).eq("date", date_str).execute()
        return bool(resp.data)

    @with_retry(error_counter=supabase_errors)
    def log_followup(self, row_number: int, action: str, date_str: str, first_name: str, company: str):
    ...
    def get_today_followups(self, date_str: str) -> list:
        resp = supabase.table("followup_logs").select("metadata").eq("date", date_str).execute()
        return [f"{r['metadata']['first_name']} ({r['metadata']['company']})" for r in resp.data]
